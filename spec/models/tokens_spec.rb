module Heimdallr
  describe Token do
    ...
    subject { CreateToken.new(application: application, scopes: %w[users:create universe:implode shiba:pet], expires_at: 30.minutes.from_now).call(encode: false) }
    ...
    describe '.cache_key' do
      it 'creates a cache key using two strings' do
        expect(Token.cache_key(id: 'a', application: 'b')).to eq('b:a')
      end
    end

    describe '#revoke!' do
      let(:to_revoke) { CreateToken.new(application: application, scopes: %w[users:create universe:implode], expires_at: 30.minutes.from_now).call(encode: false) }
