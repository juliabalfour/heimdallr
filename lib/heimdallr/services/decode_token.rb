...
module Heimdallr
  ...
  class DecodeToken
    ...
    def call
      ...
      db_token = Heimdallr.cache.fetch(Heimdallr.cache_key(jwt_id, issuer)) do
        Heimdallr.token_model.where(id: jwt_id, application_id: issuer).limit(1).preload(:application).take!
      end
