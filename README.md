...
Heimdallr is a [JWT](https://jwt.io/ "JSON Web Token") authorization gem strictly designed for Rails 5 (and higher) GraphQL API projects. 
...
_Note: This generator adds a `has_many: tokens` association, if you use a different token model name, be sure to change this line!_


4) Run the token migration generator:

```shell
rails g heimdallr:token MODEL_NAME
```

_Note: This generator adds a `belongs_to: application` association, if you use a different application model name, be sure to change this line!_
...
Heimdallr.configure do |config|
  ...
  # The application model class name to use
  config.application_model = Application
  
  # The token model class name to use
